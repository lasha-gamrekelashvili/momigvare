...
export function EditPost({ post, onClose, onSuccess }: EditPostProps) {
  ...
  async function handleMarkAsSolved() {
    ...
    try {
      ...
      toast.success(post.status === "open" ? "Post marked as solved!" : "Post reopened!")
      onSuccess()
      onClose()
    } catch (error) {
      toast.error(error instanceof Error ? error.message : "Failed to update post status. Please try again.")
    } finally {
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            <div className="flex justify-between items-center">
              ...
              >
                ...
                {post.status === "open" ? "Mark as Solved" : "Reopen Post"}
              </Button>
            </div>
